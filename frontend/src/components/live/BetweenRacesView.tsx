...
export const BetweenRacesView: React.FC<BetweenRacesViewProps> = ({
  ...
  return (
    <div className="flex flex-col gap-2 sm:gap-3">
      {/* Standby Status Bar */}
      <div className="bg-[#131722] border border-white/[0.08] border-l-4 border-l-zinc-500 rounded-xl px-2.5 py-1.5 sm:px-3 sm:py-2 flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-2 text-xs font-mono shadow-sm">
        <div className="flex items-center gap-2 text-zinc-400 min-w-0">
          <Flag className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-zinc-400 shrink-0" />
          <span className="font-semibold uppercase tracking-wider text-[9.5px] sm:text-xs leading-tight">
            {lang === 'es'
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-lg bg-[#E10600]/15 hover:bg-[#E10600]/25 text-[#E10600] font-bold text-[9.5px] sm:text-[10px] border border-[#E10600]/30 transition-colors uppercase cursor-pointer w-full sm:w-auto shrink-0"
        >
      ...
      {nextRace && (
        <div className="bg-[#131722] border border-white/[0.08] border-t-2 border-t-[#E10600] rounded-xl p-3 sm:p-5 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
            <div className="flex items-center justify-between gap-2 w-full sm:w-auto">
              <span className="inline-flex items-center gap-1.5 px-2 sm:px-2.5 py-0.5 rounded-full text-[9.5px] sm:text-[10px] font-mono font-bold uppercase bg-[#E10600]/15 text-[#E10600] border border-[#E10600]/30 tracking-widest w-fit whitespace-nowrap shrink-0">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 self-start sm:self-auto shrink-0">
              {onOpenH2H && (
                <button
                  type="button"
                  onClick={onOpenH2H}
                  className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg bg-amber-400/10 hover:bg-amber-400/20 border border-amber-400/30 text-[10px] sm:text-xs font-mono font-semibold text-amber-300 hover:text-amber-200 transition-colors cursor-pointer shrink-0"
              ...
              <button
                ...
                className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg bg-white/[0.06] hover:bg-white/[0.12] border border-white/[0.1] text-[10px] sm:text-xs font-mono font-semibold text-zinc-300 hover:text-white transition-colors cursor-pointer shrink-0"
          ...
          <div className="flex items-center justify-between mt-1 flex-wrap gap-1.5 sm:gap-2">
            <div className="flex items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs text-zinc-400 font-mono">
              <MapPin className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#E10600]" />
            ...
            <span className="text-[10px] sm:text-[11px] text-zinc-400 font-mono flex items-center gap-1">
              {mode === 'track' ? (
                <>
          ...
          {timeLeft && (
            <div className="grid grid-cols-4 gap-1 sm:gap-2 mt-2.5 sm:mt-4 max-w-xs sm:max-w-sm">
              <div className="bg-[#0B0E14] border border-white/[0.08] rounded-lg p-1 sm:p-2 text-center">
                <span className="text-sm sm:text-2xl font-bold text-white font-mono tabular-nums">
                  {timeLeft.days}
                </span>
                <span className="text-[7.5px] sm:text-[9px] text-zinc-400 uppercase tracking-wider block font-mono">
              ...
              <div className="bg-[#0B0E14] border border-white/[0.08] rounded-lg p-1 sm:p-2 text-center">
                <span className="text-sm sm:text-2xl font-bold text-[#E10600] font-mono tabular-nums">
          ...
          {nextRace.sessions && nextRace.sessions.length > 0 && (
            <div className="mt-2.5 pt-2 sm:mt-4 sm:pt-3 border-t border-white/[0.08]">
              <span className="text-[9.5px] sm:text-[10px] font-mono font-bold uppercase tracking-wider text-zinc-400 mb-1.5 sm:mb-2 flex items-center gap-1.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-1 sm:gap-1.5">
                {nextRace.sessions.map((s, idx) => (
                  <div
                    key={idx}
                    className={`flex items-center justify-between p-1.5 sm:p-2 rounded-lg border text-[10px] sm:text-xs font-mono gap-1.5 sm:gap-2 ${
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 tabular-nums shrink-0">
                      <span className="text-zinc-400 text-[9.5px] sm:text-[11px] whitespace-nowrap">
                        {formatLocalDate(s.dateTime)}
                      </span>
                      <span className="font-bold text-white bg-[#131722] border border-white/[0.08] px-1 sm:px-1.5 py-0.5 rounded-md whitespace-nowrap text-[9.5px] sm:text-xs">
      ...
      {lastRace && (
        <div className="bg-[#131722] border border-white/[0.08] rounded-xl p-3 sm:p-4 shadow-sm">
          <div className="flex items-center justify-between gap-2 mb-2 sm:mb-3 min-w-0">
            <div className="flex items-center gap-2 text-xs font-mono font-bold text-zinc-400 min-w-0">
              <Trophy className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#FFD60A] shrink-0" />
            ...
            <span className="text-[9.5px] sm:text-[10px] text-zinc-400 font-mono shrink-0 whitespace-nowrap">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 sm:gap-2">
            {lastRace.podium.map((p) => {
              ...
              return (
                <div
                  key={p.position}
                  className="bg-[#0B0E14] border border-white/[0.08] rounded-lg p-2 sm:p-2.5 flex items-center justify-between font-mono"
                >
                  <div className="flex items-center gap-2 sm:gap-2.5">
                    <span
                      className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 flex items-center justify-center font-black text-[10px] sm:text-xs ${medalColor}`}
                    >
                      {p.position}
                    </span>
                    <div className="flex flex-col leading-tight">
                      ...
                      <span className="text-[9px] sm:text-[10px] text-zinc-400 break-words leading-tight">
                        {p.teamName}
                      </span>
                    </div>
                  </div>
                  <span className="text-[9.5px] sm:text-[10px] text-zinc-400 tabular-nums font-semibold">
                    {p.timeOrStatus}
                  </span>
                </div>
              );
