  ...
  return (
    <div className="min-h-screen bg-[#0B0E14] text-zinc-100 flex flex-col font-chakra antialiased">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-3 sm:px-4 py-3 flex flex-col gap-3 pb-16">
        {activeTab === 'live' && (
          <>
            {/* Live Subview Mode Switcher (Telemetría vs Entre Carreras) */}
            <div className="flex items-center justify-between bg-[#131722] border border-white/[0.08] rounded-xl p-1 select-none text-xs font-mono">
              <button
                type="button"
                onClick={() => setLiveSubView('timing')}
                className={`flex-1 py-1.5 px-3 rounded-lg font-bold text-center transition-colors cursor-pointer ${
                  liveSubView === 'timing'
                    ? 'bg-[#1C2230] text-white shadow-sm'
                    : 'text-zinc-400 hover:text-white'
                }`}
              >
                TELEMETRÍA EN VIVO / REPLAY
              </button>
              <button
                type="button"
